...
def sep(bg, fg):
    return widget.Sep(linewidth=2, foreground=fg, background=bg, padding=5)


def wicon(icon, bg, fg):
    return widget.TextBox(
        text=icon, background=bg, padding=5, foreground=fg, fontsize=20
    )
...
one_screen = [
    ...
    widget.OpenWeather(
        location="Santa Marta", format=" {main_temp} |  {humidity}", padding=5
    ),
    widget.Spacer(),
    widget.Clock(
        format="%Y-%m-%d %H:%M",
